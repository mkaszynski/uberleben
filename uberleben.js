  ...
  for (let i = 0; i < map_size; i++) {
      ...
      for (let j = 0; j < map_size; j++) {
        ...
        if (-0.5 < height2 && height2 < 0.5) {
          if (-0.45 < height2 && height2 < 0.45) {
            if (height < 3) {
              column.push([i, j, WATER, 0, leave]);
            } else {column.push([i, j, WATER, 0, STONE]);}
          } else {
            if (height < 3) {
              column.push([i, j, SEAGRASS, 0, leave]);
            } else {column.push([i, j, SEAGRASS, 0, STONE]);}
          }
        } else if (height > 3) {
          if (height > 6) {
          if (Math.random() < 0.012) {
            column.push([i, j, ALUMINUM_ORE, 0, DARKSTONE]);
          } else if (Math.random() < 0.008) {
            column.push([i, j, RHODIUM_ORE, 0, DARKSTONE]);
          } else if (Math.random() < 0.004) {
            column.push([i, j, TUNGSTEN_ORE, 0, DARKSTONE]);
          } else {
            column.push([i, j, DARKSTONE, 0, DARKSTONE]);
          }
          } else {
...
let TIN_BLOCK = 82;
let IRON_BLOCK = 83;
let ALUMINUM_BLOCK = 84;
let TUNGSTEN_BLOCK = 85;
let GOLD_BLOCK = 86;
let RHODIUM_BLOCK = 87;
let COAL_CHUNK = 88;
let ICE = 89;
let WATER_LAMP = 90;
let SEAGRASS = 91;
...
let dark_blocks = [LOG, STONE, PLANKS, COPPER_ORE, TIN_ORE, WORKBENCH, COMPRESSED_GRASS, COMPRESSED_WHEAT, IRON_ORE, ALUMINUM_ORE, TUNGSTEN_ORE, GOLD_ORE, RHODIUM_ORE, COAL, LEAVES, CHEST, STONE_BRICKS, SALVAGER, COPPER_BLOCK, TIN_BLOCK, IRON_BLOCK, ALUMINUM_BLOCK, TUNGSTEN_BLOCK, GOLD_BLOCK, RHODIUM_BLOCK, DOOR, DARKSTONE];
...
let waters = [WATER, ICE];
let water_blocks = [BRIDGE, WATER_LAMP, SEAGRASS];
...
crafts.push([[[DARKSTONE, 0, 0], [0, 0, 0], [0, 0, 0]], [[STONE, 0, 0], [STONE, 0, 0], [0, 0, 0]], 100]);
...
crafts.push([[[SEAGRASS, SEAGRASS, 0], [SEAGRASS, SEAGRASS, 0], [0, 0, 0]], [[COMPRESSED_GRASS, 0, 0], [0, 0, 0], [0, 0, 0]], 30]);
...
crafts.push([[[0, 0, 0], [0, 0, COAL], [0, 0, 0]], [[0, 0, 0], [COAL_CHUNK, 0, COAL_CHUNK], [0, 0, 0]], 50]);
crafts.push([[[0, 0, 0], [0, 0, COAL_CHUNK], [0, 0, 0]], [[0, 0, 0], [FLAME, 0, FLAME], [0, 0, 0]], 50]);
...
crafts.push([[[COPPER, TORCH, 0], [TORCH, TORCH, 0], [0, 0, 0]], [[WATER_LAMP, WATER_LAMP, 0], [WATER_LAMP, 0, 0], [0, 0, 0]], 50]);
...
crafts.push([[[TIN, TIN, 0], [TIN, 0, 0], [0, 0, 0]], [[TIN_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[TUNGSTEN, TUNGSTEN, 0], [TUNGSTEN, 0, 0], [0, 0, 0]], [[TUNGSTEN_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[ALUMINUM, ALUMINUM, 0], [ALUMINUM, 0, 0], [0, 0, 0]], [[ALUMINUM_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[GOLD, GOLD, 0], [GOLD, 0, 0], [0, 0, 0]], [[GOLD_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[RHODIUM, RHODIUM, 0], [RHODIUM, 0, 0], [0, 0, 0]], [[RHODIUM_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], 50]);

crafts.push([[[COPPER_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], [[COPPER, COPPER, 0], [COPPER, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[IRON_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], [[IRON, IRON, 0], [IRON, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[TIN_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], [[TIN, TIN, 0], [TIN, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[TUNGSTEN_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], [[TUNGSTEN, TUNGSTEN, 0], [TUNGSTEN, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[ALUMINUM_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], [[ALUMINUM, ALUMINUM, 0], [ALUMINUM, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[GOLD_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], [[GOLD, GOLD, 0], [GOLD, 0, 0], [0, 0, 0]], 50]);
crafts.push([[[RHODIUM_BLOCK, 0, 0], [0, 0, 0], [0, 0, 0]], [[RHODIUM, RHODIUM, 0], [RHODIUM, 0, 0], [0, 0, 0]], 50]);
...
let stones = [STONE, FURNACE, COAL, TIN_ORE, COPPER_ORE, IRON_ORE, ALUMINUM_ORE, TUNGSTEN_ORE, STONE_BRICKS, STONE_PATH, FORGE, SALVAGER, GOLD_ORE, RHODIUM_ORE, DARKSTONE];
...
function loop() {
  ...
  if (stage === "play") {
    ...
    for (let i of animals) {
      ...
      if ((Math.random() < despawn_rate && dis([i[0], i[1]], [posx, posy]) > 1000) || i[4] <= 0) {
        ...
        let m3 = false;
          ...
          if (drop > 0 && !waters.includes(set1) && !water_blocks.includes(set1)) {
    ...
    if (!open_inventory) {
          ...
          if (hit_bar >= 100) {
            ...
            if (courser > 0) {
              for (let i = 0; i < 6; i++) {
                for (let j = 0; j < 6; j++) {
                  if (inventory[i][j] == 0) {
                    if (!waters.includes(land[block_posx][block_posy][2]) && !water_blocks.includes(land[block_posx][block_posy][2])) {
                    if (land[block_posx][block_posy][2] != CHEST) {
                      ...
                      if (water_blocks.includes(land[block_posx][block_posy][2])) {
                        inventory[i][j] = land[block_posx][block_posy][2];
      ...
      if (mouse.held[0] && courser > 0 && dis([mouse.x, mouse.y], [600, 300]) < reach) {
        ...
        if (land[block_posx][block_posy][2] == 0 && !water_blocks.includes(courser)) {
          if (courser > 0) {
            music.place.currentTime = 0;
            music.place.play();
          }
        ...
        } else if (waters.includes(land[block_posx][block_posy][2]) && water_blocks.includes(courser)) {
          if (courser > 0) {
            music.place.currentTime = 0;
            music.place.play();
          }
  ...
  if (stage == "menue") {
    ...
    ctx.fillText("Version 1.6.8", 20, 50);
  ...
  if (render1) {
    for (let u = Math.floor(posx/SIZE) - 30; u < Math.floor(posx/SIZE) + 30; u++) {
      for (let v = Math.floor(posy/SIZE) - 20; v < Math.floor(posy/SIZE) + 20; v++) {
        ...
        if (i[2] == WATER && temp < 20) {land[i[0]][i[1]][2] = ICE;}
        if (i[2] == ICE && temp > 20) {land[i[0]][i[1]][2] = WATER;}
